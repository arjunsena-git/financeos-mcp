...
// ─── Server Definition ────────────────────────────────────────────────────────
const server = new Server({ name: "financeos-mcp", version: "1.0.0" }, { capabilities: { tools: {} } });
// ─── Helpers ──────────────────────────────────────────────────────────────────
    ...
    try {
        switch (name) {
            case "get_crypto_price": {
                const id = (args?.coin_id).toLowerCase().trim();
                ...
                return {
                    content: [{ type: "text", text: `${arrow(c24)} **${d.name} (${d.symbol.toUpperCase()})** — Live Price
...
_CoinGecko · ${now}_` }],
            ...
            case "get_fear_greed_index": {
                ...
                const hist = data.data.slice(1).map((d) => `  ${new Date(parseInt(d.timestamp) * 1000).toLocaleDateString("en-US", { weekday: "short" })}: ${d.value} — ${d.value_classification}`).join("\n");
                ...
                return {
                    content: [{ type: "text", text: `${emoji} **Crypto Fear & Greed Index**
...
_Alternative.me · ${now}_` }],
            ...
            case "get_stock_quote": {
                ...
                return {
                    content: [{ type: "text", text: `${arrow(chg)} **${sym}** — ${meta.exchangeName}
...
_Yahoo Finance · ${now}_` }],
            ...
            case "get_bittensor_stats": {
                ...
                const c7 = m.price_change_percentage_7d;
                return {
                    content: [{ type: "text", text: `🧠 **Bittensor (TAO) — Network & Price**
            ...
            case "get_portfolio_summary": {
                ...
                for (const h of stockH) {
                    ...
                    try {
                        ...
                        rows.push(`⚠️ **${sym}** — price unavailable`);
                    }
                }
                const pnl = totalValue - totalCost;
                const pnlPct = (pnl / totalCost) * 100;
                return {
                    content: [{ type: "text", text: `💼 **Portfolio P&L Summary**
...
_CoinGecko + Yahoo Finance · ${now}_` }],
            ...
            case "get_top_movers": {
                ...
                const row = (c) => `  • **${c.symbol.toUpperCase()}** $${fmt(c.current_price)} | ${c.price_change_percentage_24h >= 0 ? "+" : ""}${c.price_change_percentage_24h?.toFixed(2)}% | MCap $${fmt(c.market_cap, 0)}`;
                return {
                    content: [{ type: "text", text: `🚀 **Top ${limit} Gainers (24h)**
...
_Top 100 coins by MCap · CoinGecko · ${now}_` }],
            ...
            case "get_global_crypto_market": {
                ...
                return {
                    content: [{ type: "text", text: `🌐 **Global Crypto Market**
